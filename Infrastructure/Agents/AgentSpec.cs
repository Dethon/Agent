...
// value with no live collaborators, so it can be compared, logged and asserted on as data.
// Every difference between a top-level agent and a subagent is a field with a value here,
// which is what lets the build step assemble both without ever asking which one it is.
