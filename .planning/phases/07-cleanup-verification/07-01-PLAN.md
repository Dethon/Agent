...
4. Update `WebChat.Client/Services/Handlers/ChatNotificationHandler.cs`:
   ...
   - `stateManager.SetApprovalRequest(null)` -> `_dispatcher.Dispatch(new ClearApproval())`
...
2. Migrate each IChatStateManager usage:
   ...
   - `existingMessages.AddRange(completedTurns)` -> Dispatch AddMessage for each turn.
   - Implementation: Use `foreach (var turn in completedTurns) _dispatcher.Dispatch(new AddMessage(topicId, turn));`
   - Note: There is no batch AddMessages action in MessagesActions.cs. The foreach loop with individual AddMessage dispatches is the correct approach. Each dispatch is synchronous and updates the store immediately, so batching is not needed for correctness.
