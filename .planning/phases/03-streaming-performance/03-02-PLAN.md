...
must_haves:
  truths:
    ...
    - "Error messages display inline retry/continue button"
...
<objective>
Add CSS-only visual feedback for streaming: blinking cursor, typing indicator, and error recovery UI.
...
Output: CSS animations for streaming cursor and typing indicator; updated ChatMessage component with cursor display; error recovery button styling.
</objective>
...
3. **Error recovery button** - inline retry styling:
```css
...
```
  ...
  <done>CSS animations added for streaming cursor, typing indicator, error recovery UI</done>
</task>
...
- Error recovery styling is ready for error states
