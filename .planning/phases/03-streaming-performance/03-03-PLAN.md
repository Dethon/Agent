...
3. Re-subscribes when TopicId changes
4. Only renders when _streamingContent is not null
5. Isolation: this component re-renders independently of parent

Add ClearSubscriptions() method to StoreSubscriberComponent if not present (protected method that clears CompositeDisposable).
  </action>
