...
files_modified:
  ...
  - Domain/Contracts/IAgentFactory.cs
...
must_haves:
  ...
  key_links:
    ...
    - from: "ChatHub.SendMessage"
      to: "MultiAgentFactory.Create"
      via: "GetRegisteredUsername"
      pattern: "GetRegisteredUsername"
    - from: "MultiAgentFactory.Create"
      to: "McpAgent constructor"
      via: "username parameter"
      pattern: "username"
---
...
3. **StreamingService.cs** - Find where SendMessageAsync is called and pass senderId:
...
4. **InitializationEffect.cs** - Register user after connection:
   - Inject `IChatConnectionService` and `UserIdentityStore`
...
Note: Need to cast `IChatConnectionService` to `ChatConnectionService` to access `HubConnection`, OR add `HubConnection` property to interface. Prefer adding to interface for cleaner code.
  </action>
  <verify>
Build succeeds: `dotnet build WebChat.Client/WebChat.Client.csproj`
  </verify>
  <done>
Client sends senderId with every message. Client registers user after initial connection and on every reconnection.
  </done>
</task>
...
3. **WebChatMessengerClient.cs** is already correct - it passes `sender` to ChatPrompt which flows to the agent.
...
- ChatMonitor passes ChatPrompt.Sender to agentFactory.Create as userId
  </action>
  <verify>
Build succeeds: `dotnet build Agent/Agent.csproj`
  </verify>
  <done>
ChatHub.SendMessage accepts senderId parameter. Registered username (not client-provided senderId) passed to agent.
  </done>
</task>
