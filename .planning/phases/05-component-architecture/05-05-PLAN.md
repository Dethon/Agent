...
must_haves:
  truths:
    ...
    - "All child components receive minimal or no props"
...
Purpose: ChatContainer (305 lines) becomes a simple layout component (~80 lines) that renders child components. All business logic moves to effects. Child components get data from stores directly.
...
<task type="auto">
  ...
  <action>
Refactor ChatContainer.razor to minimal composition root:
...
Key changes:
1. Remove ALL injections except IDispatcher and TopicsStore (for Initialize)
...
9. Child components receive NO props - they subscribe to stores directly

The @using statements can be simplified to just what's needed.

Note: MessageList still needs work for store subscription - it will be addressed if time permits or in next phase. For now, pass minimal required props or have it subscribe to stores.

Actually, review MessageList requirements:
- It needs TopicId to know which messages to show
- It has @ref for SetShouldAutoScroll

For now, keep TopicId as a parameter but get it from TopicsStore subscription:
```razor
<MessageList TopicId="@_selectedTopicId" />
...
This keeps ChatContainer under 50 lines while MessageList still works.
  </action>
  <verify>
Build succeeds: `dotnet build WebChat.Client`
...
ChatContainer.razor does NOT contain multiple event handler methods
  ...
  <done>ChatContainer is thin composition root under 100 lines</done>
</task>
