...
must_haves:
  truths:
    - "Developer can dispatch actions via IDispatcher without coupling to specific stores"
    - "Developer can create components inheriting StoreSubscriberComponent with automatic subscription cleanup"
    - "Developer can use Subscribe() overloads with selectors for fine-grained state observation"
    - "IDispatcher is resolvable from DI container"
  artifacts:
...
2. Add a temporary verification in Program.cs after building services:
   ```csharp
   // Verify DI registration (remove after verification)
   using var scope = app.Services.CreateScope();
   var dispatcher = scope.ServiceProvider.GetRequiredService<IDispatcher>();
   Console.WriteLine($"IDispatcher resolved: {dispatcher.GetType().Name}");
   ```
3. Run `dotnet run --project WebChat` and confirm console shows "IDispatcher resolved: Dispatcher"
4. Remove the temporary verification code after confirming
  </verify>
  <done>Dispatcher registered in DI container as both concrete type and IDispatcher interface, verified resolvable via GetRequiredService</done>
</task>
...
4. IDispatcher is resolvable via GetRequiredService<IDispatcher>()
...
- IDispatcher can be resolved from DI container (verified via GetRequiredService)
