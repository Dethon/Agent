...
must_haves:
  truths:
    - "Store<TState> can hold immutable state and notify subscribers on changes"
    - "Actions are typed via IAction marker interface"
    - "State is accessed via Store.State property"
    - "Subscriptions receive state updates via IObservable"
  artifacts:
