...
`appsettings.json` / `appsettings.Development.json` is where new configuration lives by default. `DockerCompose/docker-compose.yml`'s `environment` block and `DockerCompose/.env` are not a mirror of every setting — they exist only for:
...
```bash
# Linux / WSL
docker compose -f DockerCompose/docker-compose.yml -f DockerCompose/docker-compose.override.linux.yml -p jackbot up -d --build agent webui observability mcp-vault mcp-sandbox mcp-websearch mcp-idealista mcp-homeassistant mcp-library mcp-channel-signalr mcp-channel-telegram mcp-channel-servicebus mcp-channel-voice mcp-scheduling mcp-printer mcp-timers lemonade tse-extractor qbittorrent jackett redis caddy camoufox homeassistant music-assistant

# Windows
docker compose -f DockerCompose/docker-compose.yml -f DockerCompose/docker-compose.override.windows.yml -p jackbot up -d --build agent webui observability mcp-vault mcp-sandbox mcp-websearch mcp-idealista mcp-homeassistant mcp-library mcp-channel-signalr mcp-channel-telegram mcp-channel-servicebus mcp-channel-voice mcp-scheduling mcp-printer mcp-timers lemonade tse-extractor qbittorrent jackett redis caddy camoufox homeassistant music-assistant
```
...
Countdown timers live in **`McpServerTimers`** (container `mcp-timers`, port 6016), a **pure filesystem tool server** (McpServerPrinter shape) that exposes `filesystem://timers` (mount `/timers`). Any agent mounts it via `mcpServerEndpoints` — it is **not** a channel, because a timer just *rings*, it does not message the agent. The `TimerFileSystem` engine (`Domain/Tools/Timers/Vfs/`) and the non-durable `InMemoryTimerStore` run here, plus `TimerFireService` (a `BackgroundService` polling for due timers).

The three things a timer needs that only exist in the voice hub are reached over HTTP, so the hub stays the single source of truth for ringing and satellite resolution:
- **Fire** — `TimerFireService` POSTs a due timer to the hub's `POST /api/voice/announce` (`HttpInsistentAnnouncer` implementing `IInsistentAnnouncer`). Ringing runs on the hub's live Wyoming satellite sessions.
- **Dismiss** — `exec dismiss.sh` POSTs to `POST /api/voice/dismiss` (`HttpAlertDismisser` implementing `IAlertDismisser`), which cancels the hub's live `ActiveAlertRegistry` CancellationTokenSources. Wake/button dismissal stays hub-local.
- **Validate target** — `CreateAsync` resolves the target via `GET /api/voice/satellites` (roster, fetched fresh per call — it only changes on hub restart, exactly when a cache would go stale) + `POST /api/voice/satellites/resolve` (`HttpSatelliteCatalog` implementing `ISatelliteCatalog`). Resolution is **never** done in the timers process — the hub's `SatelliteRegistry` dual-keys rooms on `Room` and `DisplayLocation`, so forwarding keeps create-time validation byte-identical to fire-time routing. All three endpoints share `VoiceHubAuth` (token via `X-Announce-Token`/`Announce__Token`; `BindToLoopbackOnly` must stay false so the separate container can reach them). `HttpClient.BaseAddress` comes from `VoiceHub__BaseUrl`.

`IAlertDismisser`, `ISatelliteCatalog` and `IInsistentAnnouncer` live in `Domain/Contracts` and are async precisely so the same `TimerFileSystem`/`TimerFireService` code runs unchanged whether the implementation is in-process or HTTP. The `timers_prompt` (`Domain/Prompts/TimerPrompt.cs`) teaches the `/timers` idiom and embeds the live hub roster at prompt-fetch time (`TimersSystemPrompt` asks `ISatelliteCatalog` with a 2s cap and fails open to the roster-less static text, so an unreachable hub can never stall or fail agent session build); its text instructs the LLM to branch on the channel (on a voice turn, default the target to the speaking room; on any other channel, offer the roster rooms and ask which one).
