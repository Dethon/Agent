...
- **Validate target** — `CreateAsync` resolves the target via `GET /api/voice/satellites` (roster, cached) + `POST /api/voice/satellites/resolve` (`HttpSatelliteCatalog` implementing `ISatelliteCatalog`). Resolution is **never** done in the timers process — the hub's `SatelliteRegistry` dual-keys rooms on `Room` and `DisplayLocation`, so forwarding keeps create-time validation byte-identical to fire-time routing. All three endpoints share `VoiceHubAuth` (token via `X-Announce-Token`/`Announce__Token`; `BindToLoopbackOnly` must stay false so the separate container can reach them). `HttpClient.BaseAddress` comes from `VoiceHub__BaseUrl`.
