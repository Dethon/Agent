...
public class WyomingSatelliteHostTests
{
    ...
    public async Task Hub_CommandRunsOnFromTheWakeWord_UsesTheRoomLevelTheSatelliteMeasured()
    {
        ...
        var fakeSatellite = Task.Run(async () =>
        {
            ...
            var data = new JsonObject { ["rate"] = 16_000, ["width"] = 2, ["channels"] = 1 };
            async Task sendChunks(short level, int chunks)
            {
            ...
            await sendChunks(8000, 4);  // the command starts on the very first frame — no pre-roll gap
            await sendChunks(2000, 8);  // a quieter clause: 12 dB under the peak, still far above the clamp
            await sendChunks(0, 20);    // the user actually stops
