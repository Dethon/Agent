...
- **Do NOT touch** `McpChannelVoice`'s audio path or the satellite's **capture** (`arecord`/wake-tone) path — the design depends on both staying byte-for-byte unchanged.
...
5. **Phase-2 GATE (do this BEFORE trusting the mixer):** with `dmix`/`softvol` installed and the satellite's `aplay` on `duckmix`, speak to the satellite and play a cue/wake — confirm **TTS onset, cue beeps, and keep-warm sound identical to a voice-only unit**. If onset is degraded, tune `--start-delay`/`-F` (and the `dmix` `period`/`buffer`) before proceeding. Capture/wake must be unaffected (it is on direct `plughw`).
