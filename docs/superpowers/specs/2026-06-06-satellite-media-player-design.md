...
> **⚠️ Superseded (mechanism only) — historical record.** The satellite fleet now runs the in-house Rust **`nabu-satellite`** binary (`satellite/` crate; embedded tract wake detection), **not** the Python `wyoming-satellite` firmware this spec assumes. The on-device pieces described below — `pipx install wyoming-satellite wyoming-openwakeword`, extending `scripts/provision-satellite.sh` (removed), and PipeWire *role-ducking* + `wyoming-satellite` *event hooks* — were **not** the implementation used. The realized path is `scripts/provision-satellite-rs.sh` (PipeWire mixing on the 3.5 mm jack + `snapclient`) with the Rust satellite ducking the `Music` softvol from its connection-state channel. The **design brain** (Music Assistant + Snapcast, room→`media_player.<room>` convention, the risk table, and the "why not LVA/ESPHome" decision record) was carried forward and is incorporated **by reference** — see [`2026-06-26-satellite-media-player-rust-coexistence-design.md`](2026-06-26-satellite-media-player-rust-coexistence-design.md). Kept intact as the design-time record.
