...
- [ ] A base class implements the backend contract with all twelve operations virtual.
- [ ] Every operation returns an unsupported envelope by default, and the envelope names the operation rather than the method.
- [ ] The four envelope helpers are available to subclasses.
- [ ] The glob prologue applies a base path, and a trailing slash yields directories only.
- [ ] A search regex is compiled with a match timeout; an uncompilable pattern returns an invalid-argument envelope rather than throwing.
- [ ] The search template is available to subclasses and uses the guarded regex.
- [ ] Each operation has a description hook with a generic default.
- [ ] The five existing backends are untouched and their tests still pass.
