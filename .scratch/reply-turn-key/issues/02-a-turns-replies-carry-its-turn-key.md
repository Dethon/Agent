...
- [x] The inbound channel message carries an optional turn key.
- [x] The conversation group mints a turn key when the inbound message carries none, and keeps
      the one it was given otherwise.
- [x] Every reply produced for a turn carries that turn's key, including the stream-complete
      event and error chunks.
- [x] Every reply carries whether its turn was agent-initiated, derived from the message origin.
- [x] Two turns in one conversation produce two different keys; every update of one turn carries
      the same key.
- [x] All four channel `send_reply` tools and the shared no-outbound-surface stub accept both
      new parameters and round-trip them unchanged.
- [x] Proved at two existing seams: the monitor's delivery identity tests for minting and echo,
