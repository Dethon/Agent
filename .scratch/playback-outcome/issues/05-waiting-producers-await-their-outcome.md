...
- [ ] The earcon queues its job and awaits the outcome, with its hand-rolled completion
      source and its three callback wirings gone.
- [ ] The confirmation prompt does the same, for both the question and the re-prompt after
      a misheard answer.
- [ ] A refused prompt abandons the approval, which is what the current early return on a
      rejected enqueue does.
- [ ] Both keep their cancellation tokens, and a comment records that the token now means
      "this caller has its own reason to stop waiting", not "otherwise this hangs".
- [ ] The three spin-waits in the confirmation-prompt tests become awaits on an outcome.
- [ ] At the satellite connection seam, a test proves the earcon returns when its outcome
...
- [ ] The follow-up conversation tests and the voice integration suite pass.
