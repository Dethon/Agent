...
- [x] Asking the module to resume a topic grants a lease when the topic has nothing in flight, and
      refuses when it is already resuming or already streaming.
- [x] The resume service makes one such request instead of its three separate checks, and no
      longer needs the streaming service to answer questions about state.
- [x] The effect that reacts to a pushed stream start asks the module whether the topic is already
      resuming, not the store.
- [x] The set of resuming topics is gone from the store's streaming state, and nothing reads it.
- [x] Two pushed stream starts arriving back to back for one topic resume it once.
- [x] A resume that finds no stream in progress on the server leaves the topic idle and marks
      nothing; a resume whose hub call could not be made says nothing either way, as today.
- [x] The resume service's collaborator count drops, and its tests construct it with the smaller
      set.
- [x] `dotnet test` on `Tests/Unit` is green.
- [x] The WebChat E2E suite passes: the Cancel button appears while a reply is being written and
